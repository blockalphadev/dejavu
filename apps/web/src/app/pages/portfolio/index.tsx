...
import { Settings, Download, Upload, Search, Filter, EyeOff, Eye, ArrowRight, TrendingUp, Copy, Wallet } from "lucide-react";
...
export function PortfolioPage() {
    ...
    const { address: suiAddress, isConnected: isSuiConnected, tokenBalances: suiTokenBalances, isBalanceLoading: isSuiBalanceLoading } = useSuiWallet();
    ...
    // Combine assets from deposit balance and Sui wallet
    const allAssets = useMemo(() => {
        const assets: any[] = [];
        
        // Add assets from deposit balance
        if (balance?.assets) {
            assets.push(...balance.assets);
        }
        
        // Add assets from Sui wallet
        if (isSuiConnected && suiTokenBalances) {
            Object.entries(suiTokenBalances).forEach(([key, token]) => {
                // Skip if already exists in balance.assets
                const exists = balance?.assets?.some((a: any) => 
                    a.symbol === token.symbol && a.chain === 'sui'
                );
                
                if (!exists && token.balance > 0) {
                    assets.push({
                        symbol: token.symbol,
                        name: token.name,
                        chain: 'sui',
                        balance: token.balance.toFixed(6),
                        valueUsd: '0.00', // TODO: Get USD price
                        address: suiAddress,
        ...
        return assets;
    }, [balance?.assets, suiTokenBalances, isSuiConnected, suiAddress]);
        ...
        <div className="pb-24 pt-4 px-4 w-full lg:max-w-7xl mx-auto min-h-screen text-foreground bg-background transition-colors duration-300">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 items-start">
                ...
                <div className="lg:col-span-1">
                    {(allAssets.length > 0 || isSuiBalanceLoading) && (
                        <div className="bg-card/50 border border-border/50 rounded-2xl p-4 h-[420px] flex flex-col backdrop-blur-md transition-all">
                            <div className="flex items-center justify-between mb-4 flex-shrink-0">
                                <div className="flex items-center gap-2">
                                    ...
                                    {isSuiConnected && (
                                        <div className="flex items-center gap-1 px-1.5 py-0.5 bg-primary/10 text-primary rounded text-[8px] font-medium">
                            ...
                            <div className="space-y-1 overflow-y-auto pr-1 flex-1 -mr-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent hover:scrollbar-thumb-white/20">
                                {isSuiBalanceLoading && allAssets.length === 0 ? (
                                    ...
                                    .map((asset: any) => {
                                        ...
                                        const getAssetConfig = (symbol: string, chain?: string) => {
                                        ...
                                        const config = getAssetConfig(asset.symbol, asset.chain);
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div className="font-bold text-sm text-foreground flex items-center gap-1">
                                                            {asset.symbol === 'ETHEREUM' ? 'ETH' : asset.symbol}
                                                            {asset.source === 'wallet' && (
                                                                <Wallet className="w-3 h-3 text-primary" title="From Wallet" />
                                                            )}
                                                        </div>
