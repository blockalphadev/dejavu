...
import { depositApi, userApi } from '../../services/api';
...
export function WithdrawModal({ isOpen, onClose, onSuccess }: WithdrawModalProps) {
    ...
    const [addressSource, setAddressSource] = useState<'external' | 'custom'>('external'); // 'external' or 'custom'
    ...
    // Fetch external wallets when modal opens
    useEffect(() => {
        if (isOpen && authenticated) {
            const fetchExternalWallets = async () => {
                try {
                    ...
                    // Auto-select external wallet for current chain if available
                    const externalWalletForChain = response.wallets?.find(w => w.chain === selectedChain.id);
                    if (externalWalletForChain && addressSource === 'external') {
                        setAddress(externalWalletForChain.address);
                    }
                } catch (err) {
                    console.error('Failed to fetch external wallets:', err);
                    // Don't show error - just proceed without external wallets
                }
            };
            
            fetchExternalWallets();
        }
    }, [isOpen, authenticated, selectedChain.id, addressSource]);
                    ...
                    {step === 'input' && (
                        <div className="space-y-8">
                            ...
                            <div className="space-y-3">
                                ...
                                {/* Address Source Selection (if external wallets available) */}
                                {externalWallets.length > 0 && externalWallets.some(w => w.chain === selectedChain.id) && (
                                    <div className="flex gap-2">
                                        <button
                                            type="button"
                                            onClick={() => {
                                                setAddressSource('external');
                                                const externalWallet = externalWallets.find(w => w.chain === selectedChain.id);
                                                if (externalWallet) {
                                                    setAddress(externalWallet.address);
                                                }
                                            }}
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (addressSource === 'external') {
                                                setAddressSource('custom'); // Switch to custom if user edits
                                            }
                                        }}
                                        ...
                                        disabled={addressSource === 'external' && externalWallets.some(w => w.chain === selectedChain.id && w.address === address)}
                                    />
